...
const StructuredData = ({ locale }: StructuredDataProps) => {
  const getPersonData = () => {
    const baseData = {
      ...
      description:
        locale === "en"
          ? "PhD candidate at Kyoto University researching technology for language education. Developing data-informed learning support for second language acquisition and Japanese language teaching methodologies."
          : locale === "zh"
            ? "京都大学博士研究生，专注语言教育领域的技术研究。开发数据驱动的第二语言习得支持系统，推进日语教育方法论的持续优化。"
            : "京都大学大学院博士課程で言語教育とテクノロジーの連携を研究。学習データの分析を活用した日本語教育システムの開発と、外国語習得支援技術の改善に取り組んでいます。",
      url: "https://ryosh.in",
  ...
  const getWebsiteData = () => ({
    ...
    description:
      locale === "en"
        ? "Personal portfolio of LIANG ZHEN (RYO SHIN), PhD student specializing in language education technology and second language acquisition"
        : locale === "zh"
          ? "梁震个人作品集，京都大学博士研究生，专注于语言教育技术与第二语言习得研究"
          : "梁震（りょう しん）のポートフォリオサイト。京都大学博士課程で言語教育技術と第二言語習得の研究に専念",
    inLanguage: [locale],
    potentialAction: {
      "@type": "SearchAction",
      target: {
