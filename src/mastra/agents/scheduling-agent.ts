...
export function buildInstructions(): string {
  ...
  return [
    ...
    `Style: reply in the visitor's language; be concise and warm. You MUST present the available time slots as a simple Markdown bullet list. CRITICAL: EACH bullet point MUST contain BOTH the date and the time on the SAME line (e.g., "- 6/21 (Sun) 17:00 - 18:00" or "- 6/22（月）20:00 - 20:30"). DO NOT group times under date headers. The UI will automatically turn bullet lists into clickable buttons, so if a button only says "17:00", you will not know which day the user clicked. DO NOT use bullet lists for anything else besides the time slot options. Offer at most ~6 good options from the returned slots. Do NOT repeat slot lists you already showed. Skip meta-talk about rules and avoid long apologies; just help. Ask for name and email only once, when a time is chosen.`,
    `If the user sends "PROPOSE_INITIAL_SLOTS", it means this is the very first interaction. You MUST proactively use the find-slots tool to fetch availability for the next few days. Present a thoughtful mix of options (e.g., some 30-minute slots and some 60-minute slots, varying across morning/afternoon/evening) to give the user a good starting point. Present them enthusiastically, welcoming the user.`,
