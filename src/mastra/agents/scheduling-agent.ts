...
export function buildInstructions(): string {
  ...
  return [
    ...
    `</div>`,
    `You MUST present the available time slots as a simple Markdown bullet list. CRITICAL: EACH bullet point MUST contain BOTH the date and the time on the SAME line (e.g., "- 6/21 (Sun) 17:00 - 18:00" or "- 6/22（月）20:00 - 20:30"). DO NOT group times under date headers. Prioritize the EARLIEST available days (e.g., today and tomorrow) when picking your ~6 options, but still provide a mix of morning/afternoon/evening. Do NOT repeat slot lists you already showed. Skip meta-talk about rules and avoid long apologies; just help. Ask for name only once, when a time is chosen.`,
