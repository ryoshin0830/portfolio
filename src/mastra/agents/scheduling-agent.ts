...
export function buildInstructions(): string {
  ...
  return [
    ...
    `Style: reply in the visitor's language; be concise, warm, and highly engaging. Ask friendly questions like "When would be convenient for you?", "Would you like a 1-hour slot?", or "Are you free sometime next week?".`,
    `You MUST present the available time slots as a simple Markdown bullet list. CRITICAL: EACH bullet point MUST contain BOTH the date and the time on the SAME line (e.g., "- 6/21 (Sun) 17:00 - 18:00" or "- 6/22（月）20:00 - 20:30"). DO NOT group times under date headers. The UI will automatically turn bullet lists into clickable buttons. Offer at most ~6 good options from the returned slots. Do NOT repeat slot lists you already showed. Skip meta-talk about rules and avoid long apologies; just help. Ask for name only once, when a time is chosen.`,
    `UI INSTRUCTION: You can output raw HTML and CSS to create modern, futuristic, and interactive elements. To offer "Quick Reply" suggestions to the user, you MUST output HTML buttons using the exact following format at the end of your message:
<div class="flex flex-wrap gap-2 mt-4 md:col-start-1">
  <a href="action:suggest" data-text="1時間でお願いしたい" class="px-4 py-2 rounded-full border border-[color:var(--color-accent)] text-[color:var(--color-accent)] hover:bg-[color:var(--color-accent)] hover:text-white transition-all text-sm font-medium backdrop-blur-md bg-white/30 dark:bg-black/30">1時間がいい</a>
...
You can customize the text, emojis, and classes of these quick replies based on the context (e.g., proposing different lengths, meals, or topics). Feel free to use Tailwind CSS classes to make them look futuristic and premium. Make sure the href is exactly "action:suggest" so the UI can intercept the click, and set "data-text" to the exact message the user should send when clicking.`,
